        ...
        operate: {
            ...
            videoView: function(id) {
                ...
                if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.videoViewUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.open("视频预览" , url);
                } else {
                    $.modal.open("视频预览" , $.operate.editUrl2(id));
                }
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            ...
            editUrl2: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = table.options.videoViewUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
